...
Contrôles par carte (body) :
  1. les 5 sections obligatoires, dans l'ordre :
     « Contexte & Objectif », « Critères d'acceptation », « DoR & DoD »,
  ...
  4. garde-fous : au moins un interdit explicite (mot-clé d'interdiction)
...
def lint(task: dict) -> list[str]:
    ...
    # 1. sections obligatoires, dans l'ordre
    positions = []
    for label, pat in SECTION_PATTERNS:
        m = pat.search(body)
        if not m:
            issues.append(f"section manquante : « {label} »")
        else:
...
def main() -> int:
    ...
    for t in tasks:
        ...
        if problems:
            bad += 1
            print(f"[pj-lint] {t['id']} « {t['title'][:60]} » ({t.get('status')}) — NON CONFORME :")
            for p in problems:
                print(f"           - {p}")
    if bad:
