#!/usr/bin/env python3
"""pj-repo-watch — détecte les nouveaux repos <owner> et les onboard sur le pipeline pj.

Déterministe, aucun LLM. Onboard en 2 PHASES (idempotent, retry au tick) :

  Phase A — dès la création du repo (AUCUN contenu requis) :
    wrappers bridge+deployer + crons pj-bridge-<slug>/pj-deploy-<slug> + board
    pj-<slug>. Dès lors le pont poll le repo : les issues ouvertes sont importées
    en triage (0 LLM) et attendent le code. Marqueur : le board existe.

  Phase B (dès le premier commit) :
    branche dev sur GitHub (depuis la default), clone --branch dev dans
    ANCHOR_ROOT/<repo>, set-default-workdir (ancrage worktree). Marqueur : le clone.

Pourquoi 2 phases : un repo vide (409 sur /commits, size 0) n'est ni clonable ni
branchable — MAIS l'import d'issues n'a pas besoin de git. Différer tout l'onboard au
premier commit laisse les issues du repo sans aucune automation (panne silencieuse) :
seul le travail de worktree doit attendre le contenu.

stdout : une ligne par action réelle ; tick muet si rien de nouveau (cron --no-agent).
Variables : PJ_WATCH_VERBOSE=1, DRY_RUN=1. Adapter GH_REPO_OWNER / ANCHOR_ROOT /
PROFILE_HOME au déploiement. Copie live : <profil orchestrateur>/scripts/.
"""
...
def phase_b(repo: str, full: str, slug: str) -> bool:
    ...
    if not os.path.isdir(anchor):
        rc = sh(["git", "clone", "--branch", "dev", "-q",
                 f"https://github.com/{full}.git", anchor], timeout=300)
        if rc.returncode != 0:
