#!/usr/bin/env python3
"""kanban-card-lint — vérifie qu'une carte kanban porte le contrat de spec attendu (0 LLM).

Contrôles (body de la carte) :
  ...
  2. bloc BDD : « Fonctionnalité/Feature: » + ≥2 « Scénario: » (nominal + limite/erreur)
     avec ≥3 étapes Étant donné/Quand/Alors
  3. DoR ET DoD présents
  4. garde-fous : au moins un interdit explicite
  5. dimensionnement : signaux de dépassement (nb de fichiers cités, formulations
     « refonte complète / tout le projet », body très long) sans découpage explicite

Usage :
  kanban-card-lint.py --board <slug> [--all | --task <id>] [--status todo,ready]

Sortie : une ligne par carte non conforme ; stdout vide = conforme (tick muet).
Code retour : 0 conforme, 1 non conforme, 2 erreur d'exécution.

À appeler AVANT un gate humain de validation : c'est ce linter (pas la consigne dans
le SOUL) qui rend le contrat de format opposable.

Exclusions — sans elles le linter produit des faux positifs en masse : cartes de PROCESS
(étapes du pipeline, ex. t1..t6), racine importée dont le body est la source amont
(« Importé depuis »), et cartes done/archivées.
"""
...
# Préfixes de titres de cartes de PROCESS (à exclure) — adapter au pipeline.
PROCESS_TITLE_RE = r"^t[1-6]\b"
...
SPLIT_SIGNAL = re.compile(r"\binvest\b|\bslice\b|\bsous[-\s]carte|\bd[eé]coup", re.I)
...
def lint(task):
    ...
    if not GHERKIN_FEATURE.search(body):
        issues.append("BDD : pas de bloc « Fonctionnalité:/Feature: »")
    n_scen = len(GHERKIN_SCENARIO.findall(body))
    if n_scen < 2:
        issues.append(f"BDD : {n_scen} scénario(s), minimum 2 (nominal + limite/erreur)")
    n_steps = len(GHERKIN_STEPS.findall(body))
    if n_steps < 3:
        issues.append(f"BDD : {n_steps} étape(s) Étant donné/Quand/Alors, minimum 3")
    ...
    if n_files > 5 and not SPLIT_SIGNAL.search(body):
        issues.append(f"dimensionnement : {n_files} fichiers cités sans découpage explicite")
    if BLOAT.search(body) and not SPLIT_SIGNAL.search(body):
        issues.append("dimensionnement : formulation « tout/refonte complète » sans découpage")
    if len(body) > 12000:
        issues.append(f"dimensionnement : body très long ({len(body)} car.) — découper ?")
    return issues
...
def main():
    ...
    try:
        ...
        print(f"[card-lint] ERREUR: {e}")
        return 2

    def is_spec(t):
        title = (t.get("title") or "").strip()
        if re.match(PROCESS_TITLE_RE, title.lower()):
            return False
        if t.get("status") in ("done", "archived"):
            return False
        if "importé depuis" in (t.get("body") or "").lower():
            return False  # racine importée (body = source amont, pas une spec)
        return True
    ...
    for t in tasks:
        ...
        if problems:
            bad += 1
            print(f"[card-lint] {t['id']} « {(t.get('title') or '')[:60]} » ({t.get('status')}) — NON CONFORME :")
            for p in problems:
                print(f"           - {p}")
    if bad:
        print(f"[card-lint] {bad} carte(s) non conforme(s) sur {a.board} — compléter avant validation.")
        return 1
    return 0
