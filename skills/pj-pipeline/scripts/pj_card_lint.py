...
Contrôles par carte (body) :
  ...
  2. bloc Gherkin : « Fonctionnalité/Feature: » + ≥2 « Scénario/Scenario: »
     (nominal + limite/erreur) avec ≥3 étapes Étant donné/Quand/Alors
     (ou Given/When/Then)
...
cartes françaises déjà en base.
...
SECTION_PATTERNS = [
    ...
    ("Context & Objective", re.compile(r"^\s*#*\s*(?:\d\.\s*)?(?:contexte|context)\s*&\s*(?:objectif|objective)", re.I | re.M)),
    ("Acceptance criteria", re.compile(r"^\s*#*\s*(?:\d\.\s*)?(?:crit[eè]res\s+d'acceptation|acceptance\s+criteria)", re.I | re.M)),
    ("DoR & DoD", re.compile(r"^\s*#*\s*(?:\d\.\s*)?DoR\s*&\s*DoD", re.I | re.M)),
    ("Technical considerations", re.compile(r"^\s*#*\s*(?:\d\.\s*)?(?:consid[eé]rations\s+techniques|technical\s+considerations)", re.I | re.M)),
    ("Out of scope", re.compile(r"^\s*#*\s*(?:\d\.\s*)?(?:hors[-\s]?scope|out[-\s]?of[-\s]?scope)", re.I | re.M)),
]
...
def lint(task):
    ...
    for label, pat in SECTION_PATTERNS:
        m = pat.search(body)
        if not m:
            issues.append("missing section: « " + label + " »")
        else:
...
def main():
    ...
    for t in tasks:
        ...
        if problems:
            bad += 1
            print(f"[pj-lint] {t['id']} « {(t.get('title') or '')[:60]} » ({t.get('status')}) — NOT COMPLIANT:")
            for p in problems:
                print(f"           - {p}")
    if bad:
