#!/usr/bin/env python3
"""pj_coverage_gate — enforcement de la couverture de tests PAR FICHIER sur le périmètre
DES MODIFICATIONS (patch coverage), pas sur tout le repo.

Pourquoi ce scope : un seuil global par fichier fait échouer une carte sur du code
qu'elle n'a PAS touché — le worker se retrouve bloqué pour un travail qui n'est pas le
sien. Le gate d'une carte se limite à SES fichiers (diff de la branche), et un seuil
global sert de garde-fou anti-régression, sans conditionner la carte.

Deux modes :
  --diff-base origin/dev   ne contrôle QUE les fichiers modifiés depuis cette base
  (sans --diff-base)       contrôle tout le rapport (non-régression globale)

Vérification obligatoire avant de s'y fier : le gate doit sortir ROUGE au premier
lancement sur un repo dont les tests sont partiels. Un gate vert d'emblée ne teste rien
(vérifier alors le scope et le contenu réel du rapport).

Sources du rapport : `coverage json` (coverage.py, Python) ; pour les repos TypeScript,
préférer les seuils natifs vitest (`test.coverage.thresholds.perFile` dans la config) et
n'utiliser ce script que pour la lecture du périmètre.

Usage :
  pj_coverage_gate.py --json coverage.json --diff-base origin/dev --repo /chemin/worktree
  pj_coverage_gate.py --json coverage.json --run "pytest --cov --cov-report=json" --min 80
...
# Fichiers déclaratifs / non exécutables : les exiger à 80 % réclame des tests pour du
# code qui n'a pas de comportement (types, configs, version, l'arborescence de tests).
...
def violations(pct: dict, minimum: float, ignore=(), scope=None) -> list:
    """[(fichier, pct)] sous le seuil, restreint à `scope` (chemins) si fourni."""
    ign = set(ignore or ())
    out = []
...
def main() -> int:
    ...
    if a.run:
        ...
        if r.returncode != 0:
            # Un run rouge n'est PAS un échec du gate : la couverture peut exister alors
            # que des tests échouent — c'est la carte de convergence qui juge le reste.
